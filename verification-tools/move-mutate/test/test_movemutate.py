...
class TestShadowTriage(unittest.TestCase):
    """Shadow triage annotates survivors; it must never suppress or reassure.

    The three labelled shapes are pinned to real packages measured 2026-08-30:
    integer-mate's pure-construction guard (mutant KILLED), Volo's division and
    cast guards (SURVIVED), Praxis's table::add replay guard (SURVIVED).
    """
    ...
    def setUp(self):
        ...
        with open(os.path.join(self.tmp, "Move.toml"), "w") as fh:
            fh.write("[package]\nname='fx'\n")
        with open(os.path.join(self.tmp, "sources", "s.move"), "w") as fh:
            fh.write(fixture("shadow_shapes.move"))
    ...
    def _labels(self):
        files = derive.iter_sources(self.tmp)
        muts, _s, _e = derive.derive_parsed(self.tmp, files, ["DEL"])
        muts = shadow.annotate(self.tmp, muts)
        out = {}
        text = fixture("shadow_shapes.move")
        functions = movelex.find_functions(text)
        for m in muts:
            a = [x for x in movelex.find_asserts(text)
                 if movelex.line_of(text, x["start"]) == m["line"]][0]
            fn = movelex.enclosing_function(functions, a["start"])
            out[fn["name"]] = m["shadow"]
        return out

    # --- the three measured cases, one test each -------------------------

    def test_integermate_pure_construction_is_no_fallback(self):
        """integer-mate i128::from — tail is a struct literal, mutant was KILLED."""
        sh = self._labels()["pure_construction"]
        self.assertEqual(sh["label"], shadow.NO_FALLBACK)
        self.assertIn("return normally", sh["evidence"])

    def test_volo_division_guard_is_correlated(self):
        """Volo math::mul_div — `/ z` aborts on the guard's own condition."""
        sh = self._labels()["divisor_guard"]
        self.assertEqual(sh["label"], shadow.CORRELATED)
        self.assertIn("z", sh.get("shared_symbols", []))

    def test_volo_cast_guard_is_correlated(self):
        """Volo math::mul_div — the `as u64` cast aborts on the bounded value."""
        sh = self._labels()["overflow_cast_guard"]
        self.assertEqual(sh["label"], shadow.CORRELATED)
        self.assertIn("r", sh.get("shared_symbols", []))

    def test_praxis_replay_guard_is_correlated(self):
        """Praxis agent_registry::register — table::add aborts on duplicate key."""
        sh = self._labels()["replay_guard"]
        self.assertEqual(sh["label"], shadow.CORRELATED)
        self.assertIn("table::add", sh["evidence"])

    # --- conservatism ----------------------------------------------------

    def test_unfollowed_call_is_unknown_never_a_guess(self):
        """A call we do not follow must be UNKNOWN, never `no-fallback-abort`."""
        sh = self._labels()["opaque_call"]
        self.assertEqual(sh["label"], shadow.UNKNOWN)

    def test_guard_as_last_statement_has_no_tail(self):
        sh = self._labels()["guard_is_last"]
        self.assertEqual(sh["label"], shadow.NO_FALLBACK)

    def test_no_fallback_is_never_claimed_when_anything_could_abort(self):
        """The sound label must not appear on any tail containing a construct
        that can abort. This is the false-confidence direction."""
        for name, sh in self._labels().items():
            if sh["label"] != shadow.NO_FALLBACK:
                continue
            self.assertIn(name, ("pure_construction", "guard_is_last"),
                          "%s must not be labelled %s" % (name, shadow.NO_FALLBACK))

    def test_non_deletion_mutations_are_not_applicable(self):
        """The guard still exists under CMP/BND, so shadow reasoning does not apply."""
        files = derive.iter_sources(self.tmp)
        muts, _s, _e = derive.derive_parsed(self.tmp, files, ["CMP"])
        muts = shadow.annotate(self.tmp, muts)
        self.assertTrue(muts)
        for m in muts:
            self.assertEqual(m["shadow"]["label"], shadow.NOT_APPLICABLE)

    # --- the standing guarantees -----------------------------------------

    def test_annotation_never_suppresses_a_mutation(self):
        files = derive.iter_sources(self.tmp)
        muts, _s, _e = derive.derive_parsed(self.tmp, files, operators.CLASSES)
        before = [m["id"] for m in derive.finalize(list(muts))]
        after = [m["id"] for m in derive.finalize(shadow.annotate(self.tmp, muts))]
        self.assertEqual(before, after)

    def test_annotation_cannot_move_the_mutation_set_hash(self):
        """A published number must survive the tool gaining an opinion."""
        files = derive.iter_sources(self.tmp)
        muts, _s, _e = derive.derive_parsed(self.tmp, files, ["DEL"])
        muts = derive.finalize(muts)
        before = derive.set_hash(muts)
        after = derive.set_hash(shadow.annotate(self.tmp, muts))
        self.assertEqual(before, after)

    def test_every_mutation_gets_a_label_from_the_known_set(self):
        files = derive.iter_sources(self.tmp)
        muts, _s, _e = derive.derive_parsed(self.tmp, files, operators.CLASSES)
        for m in shadow.annotate(self.tmp, muts):
            self.assertIn(m["shadow"]["label"], shadow.LABELS)
            self.assertTrue(m["shadow"]["evidence"])

    def test_label_meaning_refuses_to_read_as_a_verdict(self):
        self.assertIn("not why a mutant survived", shadow.LABEL_MEANING)
        self.assertIn("untested guard is an untested guard", shadow.LABEL_MEANING)

    def test_survivors_are_all_rendered_whatever_their_label(self):
        """Triage annotates. It must never drop a survivor from the report."""
        results = [
            {"file": "s.move", "line": 1, "rule_id": "DEL", "original": "assert!(a, E);",
             "outcome": "survived", "proposals": [],
             "shadow": {"label": shadow.CORRELATED, "evidence": "x"}},
            {"file": "s.move", "line": 2, "rule_id": "DEL", "original": "assert!(b, E);",
             "outcome": "survived", "proposals": [],
             "shadow": {"label": shadow.NO_FALLBACK, "evidence": "y"}},
            {"file": "s.move", "line": 3, "rule_id": "DEL", "original": "assert!(c, E);",
             "outcome": "survived", "proposals": [], "shadow": {}},
        ]
        man = {"package": "/tmp/p", "derived": 3, "operator_classes": ["DEL"]}
        md = report.build_markdown(man, results, [], [])
        for line in (1, 2, 3):
            self.assertIn("s.move:%d" % line, md)
        self.assertIn("survived: **3**", md)

    def test_report_carries_the_wrong_constant_caveat(self):
        """A clean static number must never be read as assurance."""
        self.assertIn("WRONG constant", report.STATIC_SCORE_CAVEAT)
        results = [{"file": "s.move", "line": 1, "rule_id": "DEL",
                    "original": "assert!(a, E);", "outcome": "survived",
                    "proposals": [], "shadow": {"label": shadow.NO_FALLBACK,
                                                "evidence": "y"}}]
        man = {"package": "/tmp/p", "derived": 1, "operator_classes": ["DEL"]}
        self.assertIn("WRONG constant", report.build_markdown(man, results, [], []))
